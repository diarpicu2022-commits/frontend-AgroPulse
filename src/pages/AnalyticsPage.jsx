...
export default function AnalyticsPage() {
  ...
  const exportToCSV = () => {
    ...
    const rows = filtered.map(r => { const unit = r.sensorType === 'SOIL_MOISTURE' || r.sensorType === 'HUMIDITY' ? '%' : '°C'; return [new Date(r.timestamp).toLocaleString('es-CO'), r.sensorType, r.value, unit, r.source || 'ESP32'] })
  ...
  const getChartData = (sensorType) => {
    ...
    return Object.entries(byTime).map(([time, values]) => ({ time, value: (values.reduce((a, b) => a + b, 0) / values.length).toFixed(1) })).slice(-24)
  }
      ...
      <div className="print-preview">
        ...
        <div className="p-8">
          ...
          <table className="print-table">
            ...
            <tbody>{filteredReadings.slice(0, 500).map((r, idx) => <tr key={idx}><td>{new Date(r.timestamp).toLocaleString('es-CO')}</td><td>{r.sensorType}</td><td>{r.value}</td><td>{r.sensorType === 'SOIL_MOISTURE' || r.sensorType === 'HUMIDITY' ? '%' : '°C'}</td><td>{r.source || 'ESP32'}</td></tr>)}</tbody>
          </table>
            ...
            {[
              ...
              <div key={type} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                ...
                <div className="grid grid-cols-2 gap-2 mt-3 text-xs">
                  {['Mín', 'Máx', 'Promedio', 'Actual'].map((lbl, i) => {
