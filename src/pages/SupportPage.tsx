...
export default function SupportPage() {
  ...
  const getBorderColor = (status: TicketStatus): string => {
    if (status === 'OPEN')        return 'border-yellow-200'
    if (status === 'RESOLVED')    return 'border-green-200'
    if (status === 'IN_PROGRESS') return 'border-blue-200'
    ...
    <div className="space-y-5">
      ...
      {showForm && !isAdmin && (
        <form onSubmit={handleCreate} className="bg-white rounded-2xl shadow-md border-2 border-green-200 p-5 space-y-4">
          ...
          <div>
            ...
            <select value={form.priority} onChange={e => setForm({...form, priority: e.target.value as TicketPriority})} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-green-500 focus:outline-none">
        ...
        <div className="bg-white rounded-2xl shadow-md border-2 border-blue-200 p-5 space-y-4">
          ...
          <div className="bg-gray-50 rounded-xl p-4 space-y-2">
            ...
            <div className="flex gap-2 flex-wrap text-xs">
              <span className={`px-2 py-1 rounded-full font-medium ${priorityColors[selected.priority]}`}>{selected.priorityDisplay || selected.priority}</span>
          ...
          <div className="flex gap-3">
            <select value={replyStatus} onChange={e => setReplyStatus(e.target.value as TicketStatus)} className="border-2 border-gray-200 rounded-xl px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {tickets.map(t => (
            <div key={t.id} className={`bg-white rounded-2xl shadow-sm border-2 p-4 transition-all hover:shadow-md ${getBorderColor(t.status)}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    <span className="font-bold text-gray-800 text-sm">#{t.id} {t.subject}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColors[t.status]}`}>{t.statusDisplay || t.status}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${priorityColors[t.priority]}`}>{t.priorityDisplay || t.priority}</span>
                  </div>
