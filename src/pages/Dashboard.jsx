...
import { Thermometer, Droplets, Leaf, Bell, RefreshCw, Sun, Activity, ChevronDown } from 'lucide-react'
import { useAuth } from '../context/AuthContext'
import api from '../services/api'
import SensorCard from '../components/SensorCard'
import AlertsBanner from '../components/AlertsBanner'

// Display metadata for each known sensor type
const SENSOR_META = {
...
export default function Dashboard() {
  ...
  // Load greenhouses once on mount
  useEffect(() => {
  ...
  // Reload data whenever selectedGh changes
  useEffect(() => {
  ...
  const generateAutoAlerts = (readingsData, cropData) => {
    ...
    if (tempR) {
      if (tempR.value < cropData.temp_min)
        alerts.push({ type: 'TEMPERATURE', title: '❄️ Temperatura muy baja', message: `${tempR.value.toFixed(1)}°C (mín: ${cropData.temp_min}°C)`, timestamp: now })
      else if (tempR.value > cropData.temp_max)
        alerts.push({ type: 'TEMPERATURE', title: '🔥 Temperatura muy alta', message: `${tempR.value.toFixed(1)}°C (máx: ${cropData.temp_max}°C)`, timestamp: now })
    }
    const humidity = readingsData.find(r => r.sensorType === 'HUMIDITY')
    if (humidity) {
      if (humidity.value < cropData.humidity_min)
        alerts.push({ type: 'HUMIDITY', title: '🏜️ Humedad muy baja', message: `${humidity.value.toFixed(1)}% (mín: ${cropData.humidity_min}%)`, timestamp: now })
      else if (humidity.value > cropData.humidity_max)
        alerts.push({ type: 'HUMIDITY', title: '💦 Humedad muy alta', message: `${humidity.value.toFixed(1)}% (máx: ${cropData.humidity_max}%)`, timestamp: now })
    }
    const soil = readingsData.find(r => r.sensorType === 'SOIL_MOISTURE')
    if (soil) {
      if (soil.value < cropData.soil_moisture_min)
        alerts.push({ type: 'SOIL_MOISTURE', title: '🏜️ Suelo muy seco', message: `${soil.value.toFixed(1)}% (mín: ${cropData.soil_moisture_min}%)`, timestamp: now })
      else if (soil.value > cropData.soil_moisture_max)
        alerts.push({ type: 'SOIL_MOISTURE', title: '💧 Suelo muy húmedo', message: `${soil.value.toFixed(1)}% (máx: ${cropData.soil_moisture_max}%)`, timestamp: now })
    }
    return alerts
  }
  ...
  const loadData = async () => {
    if (!selectedGh) return
    try {
      const readingsData = await api.readings.list(null, 200, selectedGh.id)
      if (readingsData?.readings) {
        setReadings(readingsData.readings)

        // Chart: find best available temperature type
        const tempType = ['TEMPERATURE_INTERNAL', 'TEMPERATURE', 'TEMPERATURE_EXTERNAL']
        ...
        try {
          ...
          if (cropsData?.crops?.length > 0) {
            ...
            if (newAutoAlerts.length > 0) { setAutoAlerts(newAutoAlerts); newAutoAlerts.forEach(a => saveAutoAlert(a)) }
            else setAutoAlerts([])
          }
  ...
  // Unique sensor types present in current readings (in order of first appearance)
  const uniqueTypes = [...new Set(readings.map(r => r.sensorType).filter(Boolean))]
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Greenhouse selector — only shows when user has more than one */}
