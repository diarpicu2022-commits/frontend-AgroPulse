...
export default function AlertsPage() {
  ...
  const handleMarkRead = async (id: number) => {
    try { await alertRepository.markRead(id); loadAlerts() }
    catch (err) { alert('Error: ' + (err as Error).message) }
  }
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-3">
          ...
          <select value={form.level} onChange={e => setForm({...form, level: e.target.value as AlertLevel})} className="w-full border rounded-xl px-4 py-2 text-sm">
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs opacity-70 mt-1">
                    {a.level} · {a.timestamp ? new Date(a.timestamp).toLocaleString('es-CO') : ''}
                  </p>
                </div>
                <button onClick={() => handleDelete(a.id)} className="text-xs opacity-50 hover:opacity-100 transition-opacity ml-2">✕</button>
              </div>
            </div>
          ))}
        </div>
