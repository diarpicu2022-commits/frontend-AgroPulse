...
// Keys are read from localStorage first, then from env vars as fallback

export function getGroqKey() {
  try {
    return localStorage.getItem('agropulse_groq_key')
...
export function getGitHubToken() {
  try {
    return localStorage.getItem('agropulse_github_token')
...
export function getGemmaKey() {
  try {
    return localStorage.getItem('agropulse_gemma_key')
      || import.meta.env.VITE_GEMMA_KEY
      || ''
  } catch {
    return import.meta.env.VITE_GEMMA_KEY || ''
  }
}

// Returns { text: string, provider: string }
export async function callAI(prompt, sensorContext = '') {
